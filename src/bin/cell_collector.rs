...
impl CellCollector {
    ...
    pub fn collect<R: Rng>(
        ...
        if count < NCELLS {
            ...
            let (least_members, n) = *self.largest_by_members.keys().next().unwrap();
            ...
            let (smallest_weight, n) = *self.largest_by_weight.keys().next().unwrap();
            if weight > smallest_weight {
                self.largest_by_weight.remove(&(smallest_weight, n)).unwrap();
                let events = cell.iter().map(|(_d, e)| e.id).collect();
    ...
    pub fn event_cells(&self) -> HashMap<usize, Vec<usize>> {
        ...
        let all_cells = self.first.iter().map(|(id, events)| (*id, events))
            .chain(self.random.iter().map(|(id, events)| (*id, events)))
            .chain(self.largest_by_radius.iter().map(|((_r, id), events)| (*id, events)))
            .chain(self.largest_by_members.iter().map(|((_n, id), events)| (*id, events)))
            .chain(self.largest_by_weight.iter().map(|((_w, id), events)| (*id, events)));
