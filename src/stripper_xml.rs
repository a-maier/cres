...
use nom::{sequence::{tuple, preceded}, character::complete::{char, i32, space0, space1, multispace0, u64, multispace1}, IResult, bytes::complete::tag, combinator::{all_consuming, recognize, opt}};
use particle_id::ParticleID;
use quick_xml::events::attributes::Attribute;
use stripper_xml::normalization::Normalization;
use thiserror::Error;

use crate::{compression::{Compression, compress_writer}, traits::{Rewind, UpdateWeights}, storage::{EventRecord, CreateError, Converter, EventFileReader, FileStorageError, ErrorKind, ReadError, Utf8Error, WriteError, EventTypeInfo}, util::{trim_ascii_start, take_chars}, event::{Event, EventBuilder, Weights}, four_vector::FourVector};
...
impl Iterator for FileReader {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        self.read_raw()
            .map(|r| r.map(|record| EventRecord::StripperXml{
                record,
...
impl FileStorage {
    ...
    fn update_next_weights_helper(&mut self, weights: &Weights) -> Result<bool, ErrorKind> {
        ...
        if weights.len() > 1 {
            ...
            let mut weights = weights.iter().skip(1).map(|&wt| wt / self.weight_scale);
    ...
    for (path, out) in paths {
        ...
        if let Ok(buf) = r.fill_buf() {
            ...
            if buf.starts_with(b"<?xml") {
                ...
                match tag {
                    XMLTag::Normalization { name, scale, weight_names } => {
...
impl Display for XMLTag {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            XMLTag::Normalization { name, scale, weight_names: _ } => write!(
...
impl StripperXmlParser for Converter {
    ...
    fn parse_stripper_xml(
        ...
        let total_energy: N64 = event.outgoing_by_pid().iter()
            .map(|(_, p)| p[0])
            .sum();
